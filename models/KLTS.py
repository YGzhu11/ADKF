...
class ResidualGRUGate(nn.Module):
    def __init__(self, seq_len, num_experts):
        super(ResidualGRUGate, self).__init__()
        self.gru = nn.GRU(seq_len, num_experts, batch_first=True)
        self.fc = nn.Linear(seq_len, num_experts)

    def forward(self, x):
        gru_out, _ = self.gru(x)
        res_out = self.fc(x) + gru_out
        return res_out
...
class KANLinear_for_Predict(nn.Module):
    def __init__(self, configs):
        ...
        #     ResidualGRUGate(self.seq_len // (2 ** i), self.num_experts)
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.fusion_layers = decompose_and_fusion_layer(configs)
        ...
        self.KP = KANLinear_for_Predict(configs)
        self.projection_layer = nn.Linear(configs.d_model, 1, bias=True)

    def Loss(self, y_true, y_pred):
        return F.mse_loss(y_pred, y_true)

    def forward(self, x_enc, x_mark_enc, y_enc):
        ...
        enc_out_list = self.fusion_layers(enc_out_list)

        dec_out_list= self.KP(B, self.d_model, enc_out_list, x_list)

        dec_out = torch.stack(dec_out_list, dim=-1).sum(-1)
        dec_out = self.normalize_layers[0](dec_out, 'denorm')
        loss = self.Loss(y_enc, dec_out)
        return dec_out, loss
